...
## Night Shift Moon short-drama acceptance and episode assembly — 2026-08-12

- Completed a real 30-second, six-shot vertical short drama from the creator UI: local OpenAI-compatible story response → validated package → real ComfyUI upload → explicit fictional-character approval → one-click serial Ref2VA generation. All six five-second shots finished on the first attempt with one running and zero pending queue items.
- Added automatic per-episode assembly after the shot queue. FFmpeg inputs are ordered by the validated production package, every video and audio timestamp starts at zero, and the output is forced to constant 24 fps. The UI now shows a prominent **Play episode** link before individual shot links.
- The first assembly exposed a real timestamp defect: 717 frames / 23.9 fps despite a 30-second container. The CFR fix produced exactly 720 frames / 24.0 fps / 30.000 seconds with H.264 yuv420p video and 32 kHz stereo AAC.
- Visual acceptance confirmed the same fictional adult lead, yellow raincoat, green uniform, and red left-wrist watch across all shots. The can, red umbrella, glowing moon, moon-in-jar, and final smile story beats are visible. Known quality limits remain: package-label pseudo-text, indoor-to-storefront scene drift, oversized moon, and imperfect action literalness.
