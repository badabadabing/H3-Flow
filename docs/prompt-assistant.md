...
最简单的方法是在“AI 提示词编导”面板中填写服务地址、模型名称和 API Key。单视频提示词编导与 AI 短剧模式共享同一份本页临时配置；任一处填写后会明确显示“本页内存已就绪”，切换模式无需重填。Key 只保留在当前页面内存，刷新或关闭页面后消失，不写入项目、浏览器存储或响应正文。

“不落盘”表示密钥已接收但不会持久化，并不表示填写失败。提交失败时，界面会区分 DeepSeek 官方常见状态：401 Key 无效、402 余额不足、422 参数错误、429 频率限制和 503 服务繁忙。
