...
Switch the top navigation to **AI Short Drama**, enter the story, episode count, and duration, then complete and lock the three identity views for each main character. The **Generate all shots** action preflights assets, models, memory, VRAM, and queue state before a single background worker submits one five-second H3 unit at a time. API keys and provider configuration are excluded from browser drafts. See [short-drama-mode.md](docs/short-drama-mode.md).
