...
The current release supports text-to-video, real first-frame image-to-video, a single-image Ref2VA identity/style mode, adaptive 5–30 second plans, guarded SeedVR2 restoration, and an optional provider-neutral prompt director that rewrites a creative brief into the official MiniMax H3 Base or Ref2VA structure. It also includes a structured AI short-drama planner for 1–8 episodes: series bible, character / wardrobe / prop / location assets, episode and scene scripts, validated 5–15 second shots, a season ledger, JSON / Markdown export, and shot-level handoff into the existing H3 desk. Planning never submits ComfyUI. A guided beat builder compiles longer stories into validated five-second timecodes, while local draft recovery and a unified preflight keep creative, runtime, model, version, queue, and hardware blockers visible before submission.
...
Switch the top navigation to **AI Short Drama**, enter a theme, episode count, duration, production constraints, and your own OpenAI-compatible model configuration. The resulting local project is validated for stable IDs, resolved references, exact episode duration, and gap-free five-second beats before it is shown. API keys and provider configuration are excluded from browser drafts. See [short-drama-mode.md](docs/short-drama-mode.md).

Controlled full-season video submission remains gated. The current release hands off one validated shot at a time; real reference assets, a passing pilot, runtime checks, and a recoverable queue are still required before batch generation can be truthfully enabled. The staged plan is documented in the [AI short-drama roadmap](docs/ai-short-drama-roadmap.md).
